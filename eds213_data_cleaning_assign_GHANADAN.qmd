...
Because we know there are no negative values in our cleaned snow, water, and land cover columns, we will just delete all observations where total cover is zero, as these observations are useless. 

When total cover is greater than 100, I'd assume that this is because of a typo. Since it's difficult to go through and figure out what the exact typo was, we will just delete these observations as well.

```{r}
# remove observations where total cover is zero
cover_df <- cover_df %>% 
  filter((Total_cover != 0) | (Total_cover < 100))
```

For the remaining entries, we will attempt to replace any NAs with the appropriate number when this is the case.

```{r}
cover_df %>% 
  filter((Total_cover != 100) & (is.na(Water_cover)))
